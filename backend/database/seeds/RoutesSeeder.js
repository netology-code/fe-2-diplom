            ...
            const train = await getRandomTrain(),
              ...
              getSeatsInfo = priceInfo => {
                const classList = Object.entries( priceInfo )
                    ...
                    .map(([ key ]) => key ),
                  getCoachesByClass = classType => coaches.filter(
                    item => item.class_type === classType ),
                  getAvailableSeats = data => sum(
                    data.map( prop( 'available_seats' ))),
                  getAvailableSeatsByClass = compose( getAvailableSeats,
                    getCoachesByClass );
                // zipWith
                return zipObj(
                  classList,
              ...
              attributes = {
                ...
                have_air_conditioning: checkCoachesForProp(
